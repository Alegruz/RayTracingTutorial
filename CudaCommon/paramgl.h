...
class ParamListGL : public ParamList
{
    public:
        ...
        void Render(int x, int y, bool shadow = false)
        {
            ...
            for (std::vector<ParamBase *>::const_iterator p = mParameters.begin(); p != mParameters.end(); ++p)
            {
                if ((*p)->IsList())
                {
        ...
        {
            if ((y < m_start_y) || (y > (int)(m_start_y + (m_separation * mParameters.size()) - 1)))
            {
                m_active = false;
                return false;
            }
            ...
            if ((button==GLUT_LEFT_BUTTON) && (state==GLUT_DOWN))
            {
                ...
                for (m_current = mParameters.begin(); m_current != mParameters.end() && i > 0; m_current++, i--) ;

                //m_current = (std::vector<ParamBase *>::const_iterator)&mParameters.at(i);
#endif
        ...
        {
            if ((y < m_start_y) || (y > m_start_y + (m_separation * (int)mParameters.size()) - 1))
            {
                return false;
            }
